...
import { useDebitNotes, useCreateDebitNote, useIssueDebitNote, useCancelDebitNote } from '../../hooks/useBilling';

const DNForm = ({ form, setForm }: { form: Record<string, unknown>; setForm: (f: Record<string, unknown>) => void }) => (
  <div className="grid gap-4">
    ...
    <div><label className="block text-xs font-medium text-ink-600 dark:text-ink-300">Reason</label>
      <input type="text" placeholder="Shipping fee, correction" value={String(form.reason ?? '')}
...
export const DebitNotesPage = () => {
  useDocumentTitle('Debit Notes');
  const { data: dnData, error, isError, isFetching } = useDebitNotes();
  const notes = dnData?.items ?? [];
  const [modalOpen, setModalOpen] = useState(false);
  const [form, setForm] = useState<Record<string, unknown>>({});
  const createMut = useCreateDebitNote();
  const issueMut = useIssueDebitNote();
  const cancelMut = useCancelDebitNote();
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-ink-900 dark:text-white">Debit Notes</h1>
      ...
      <DataState isLoading={isFetching} isError={isError} error={error} empty={notes.length === 0} emptyMessage="No debit notes found.">
        <div className="overflow-x-auto rounded-lg border border-ink-200 dark:border-ink-800">
          <table className="w-full text-sm">
            ...
            <tbody>
              {notes.map((n) => (
                <tr key={n.debitNoteId} className="border-t border-ink-200 dark:border-ink-800 last:border-0">
                  <td className="px-4 py-2.5">{n.debitNoteNumber}</td>
                  ...
                  <td className="px-4 py-2.5 text-center space-x-1">
                    {n.status === 'draft' && (
                      <button onClick={() => issueMut.mutate(n.debitNoteId)} className="inline-block rounded p-1 text-green-600 hover:bg-green-50" title="Issue"><FileText size={14} /></button>
                    )}
                    {n.status === 'issued' && (
                      <button onClick={() => cancelMut.mutate(n.debitNoteId)} className="inline-block rounded p-1 text-red-600 hover:bg-red-50" title="Cancel"><XCircle size={14} /></button>
                    )}
                  </td>
                </tr>
              ))}
      ...
      {modalOpen && (
        <Modal open={modalOpen} title="New Debit Note" onClose={() => setModalOpen(false)}
          ...
          <DNForm form={form} setForm={setForm} />
        </Modal>
      )}
    </motion.div>
  );
